...
web.get('/patientlist', async (req, res) => {
  ...
  try{
    ...
    const stmt = "SELECT JSON_ARRAYAGG(JSON_OBJECT('patientid', patientid, 'name', name, 'sex', sex, 'dateofbirth', dateofbirth)) FROM patients ORDER BY patientid LIMIT ? OFFSET ?";
